...
class AnomalyData:
    cause: int  # the name of each performance anomaly
    attributes: List[int]  # list of attribute names
    ...
    @functools.cached_property
    def valid_attributes(self) -> List[int]:
        return [self.attributes[i] for i in range(0, len(self.attributes))]
    ...
    def valid_values(self) -> np.ndarray:
        ...
        for values_in_time in self.values:
            tmp.append([values_in_time[i] for i in range(0, len(self.attributes))])
        return tmp
...
class AnomalyDataset:
    causes: List[int] = data_utils.field(default_factory=list)
    ...
    def get_data_of_cause(self, cause: int) -> List[AnomalyData]:
